...
const SavedPostSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(SavedLikePostData.fulfilled, (state, action) => {
      ...
      // Update the specific item in state.data immutably
      state.data = state.data.map((item) => {
        // Check if the item's post.id matches the action.payload.id
        if (item?.post?.id === action?.payload?.id) {
          // If there's a match, update the post with the new data from action.payload
          return {
            ...item, // Copy all existing properties from the original item
            post: action.payload, // Update the post property with the new data
          };
        } else {
          // If there's no match, return the original item unchanged
          return item;
        }
      });
    });
