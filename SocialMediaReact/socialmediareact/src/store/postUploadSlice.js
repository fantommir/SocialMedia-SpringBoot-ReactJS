...
// export const getUserAllPostsWithoutPagination = createAsyncThunk(
//   "getUserAllPostsWithoutPagination",
//   async (_, { rejectWithValue }) => {
//     const token = localStorage.getItem("token");
//     try {
//       const allusersPosts = await axios.get(
//         `http://localhost:8005/main/posts/allposts`,
//         {
//           headers: {
//             Authorization: `Bearer ${token}`,
//           },
//         }
//       );
//       console.log("allUsersPosts.data ", allusersPosts.data);
//       return allusersPosts.data;
//     } catch (error) {
//       rejectWithValue(error);
//     }
//   }
...
const postUploadSlice = createSlice({
  ...
  extraReducers: (builder) => {
    // builder.addCase(getUserAllPostsWithoutPagination.pending, (state) => {
    //   state.isLoading = true;
    // });
    // builder.addCase(
    //   getUserAllPostsWithoutPagination.fulfilled,
    //   (state, action) => {
    //     console.log("pagi action.payload ", action.payload);
    //     state.isLoading = false;
    //     state.fetched = true;
    //     state.allPosts = action.payload;
    //   }
    // );
    // builder.addCase(getUserAllPostsWithoutPagination.rejected, (state) => {
    //   state.isError = true;
    // });
